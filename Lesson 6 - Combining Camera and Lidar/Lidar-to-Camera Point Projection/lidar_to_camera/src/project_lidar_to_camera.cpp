...
void projectLidarToCamera2()
{
    ...
    cv::Mat P_rect_00(3,4,cv::DataType<double>::type); // 3x4 projection matrix after rectification (intrinsic ...)
    cv::Mat R_rect_00(4,4,cv::DataType<double>::type); // 3x3 rectifying rotation to make image planes co-planar (rectify left to right camera)
    cv::Mat RT(4,4,cv::DataType<double>::type); // rotation matrix and translation vector (extrinsic ...)
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
        /* Filter out some points that are:
        ...
        // How to multiply matricies in opencv: https://stackoverflow.com/a/10948928
